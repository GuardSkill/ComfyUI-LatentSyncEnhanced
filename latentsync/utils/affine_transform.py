...
class AlignRestore(object):
    def __init__(self, align_points=3, resolution=256, device="cpu", dtype=torch.float16):
        if align_points == 3:
            ...
            # CPU float16 kernels are incomplete on some PyTorch builds.  The
            # restoration path is intentionally CPU-friendly and uses fp32.
            self.dtype = torch.float32 if self.device.type == "cpu" else dtype
    ...
    def align_warp_face(self, img, landmarks3, smooth=True):
        ...
        # Affine metadata is small and should not pin a CUDA allocation after
    ...
    def restore_img(self, input_img, face, affine_matrix, debug_frame_index=None):
        ...
        face = face.to(device=self.device, dtype=self.dtype)
        if face.ndim == 3:
    ...
    def transformation_from_points(self, points1: torch.Tensor, points0: torch.Tensor, smooth=True, p_bias=None):
        if isinstance(points0, np.ndarray):
            ...
            points2 = points0.to(device=self.device, dtype=torch.float32)

        if isinstance(points1, np.ndarray):
            points1_tensor = torch.tensor(points1, device=self.device, dtype=torch.float32)
        else:
            points1_tensor = points1.to(device=self.device, dtype=torch.float32)

        if p_bias is not None:
            p_bias = p_bias.to(device=self.device, dtype=torch.float32)
